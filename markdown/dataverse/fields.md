...
1. The *Snake Case* notation must be used in the Name.
1. Avoid abbreviations in the Display Name or Name as much as possible.
1. Make the Display Name as descriptive as possible, remembering that this value can be changed if necessary.
1. Make the Display Name unique.
1. If you need to use the same Display Name as an existing system field, change the system field Display Name to include the OOTB (Out-of-the-box) prefix.
1. If the field type is Lookup, it must end with the _id suffix.
1. If the field type is Lookup, check the Relationship name and adjust it to use the *Snake Case* notation.
1. If you have multiple fields grouped logically, use the same prefix for all of them in the Name and Display Name.

### Rationale

1. The Snake Case syntax facilitates reading the name and is aligned with the underscore character used between the publisher prefix and the name. It makes clear that the name refers to a field in the C# or TypeScript early bound generated code.
1. Abbreviations are not always understood by everybody.
1. Unique Display Names avoid confusion for developers and users alike.
1. Ending Lookup fields with the _id suffix makes it clear the field is an Entity Reference when working with code.
1. Grouping multiple related fields with the same prefix makes it easier to identify and view them as a group in the Dataverse designers.
...
- Several fields grouped logically: cspreviewduedate, cspreviewcompletedon, cspreviewlink.

#### Bad

- The same fields without a consistent prefix: cspduedatereview, cspreviewcompletedon, csplinkreview - harder to identify as a group.
...
Every field must include a description. !Field Description

### Guidelines

1. Always include a description, remembering that it will be shown as a tooltip in the user interface.
1. If the field is for internal system purposes, start the description with "System Field." and include it in the hidden Admin tab (FRM-001).

### Rationale

1. Descriptions facilitate understanding the use and purpose of any field, for both users and developers.
1. Marking system fields with a "System Field." prefix makes it immediately clear that the field is not intended for end-user interaction.
...
- "The date when the customer's last order was placed. Used to calculate inactivity alerts."
- "System Field. Stores the workflow processing status for automated monitoring."

#### Bad

- No description at all - users and developers have no context about the field's purpose.
- "Field 1" - meaningless, provides no useful information.
...
Use Global Choices (Global Option Sets) for values that are shared across multiple tables
...
- Column-level security enabled on the salary and socialsecuritynumber fields of the Employee table, with a Field Security Profile granting access only to HR team members.
