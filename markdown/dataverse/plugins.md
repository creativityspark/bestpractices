...
Plugin class names must use a namespace-based naming convention that describes the plug...

### Description

Plugin class names must use a namespace-based naming convention that describes the plugin's purpose. Do not hardcode table schema names, pipeline stages, or event names into the class name, since a single plugin class can be registered on multiple tables and stages, and its configuration may change over time. Follow this pattern: ` [AppName].Plugins.[TableName (if table-specific)].[Action/Purpose] `

### Guidelines

1. AppName: The name of the application or solution (e.g., CRM, Sales, FieldService).
1. Plugins: A fixed namespace segment indicating the plugin layer.
1. TableName *(optional)*: Include the table name as a namespace segment only when the plugin is specific to a single table.
1. Action/Purpose: A descriptive class name that explains what the plugin does.
...
- AccountPreOpUpdate_ValidateEmail - hardcodes the table schema, pipeline stage, and event into the name.
...
Always throw InvalidPluginExecutionException for business rule violations and validatio...
...
Always use ITracingService to log diagnostic information at key points in the plugin ex...
...
Never use new ColumnSet(true) when retrieving records
...
Use pre-images and post-images instead of issuing additional Retrieve calls to access f...
...
Do not use ExecuteMultipleRequest inside plugin code
...
Register plugins only for the specific messages and tables required
...
Do not use multithreading, Task
