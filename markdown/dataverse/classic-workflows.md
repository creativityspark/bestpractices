...
Workflow and Action names must follow the pattern: ` [App Prefix] - [Table Schema Name]...

### Description

Workflow and Action names must follow the pattern: ` [App Prefix] - [Table Schema Name] - [Event Name Code(s)] - [Action/Purpose] `

### Guidelines

1. App Prefix: A short prefix (3 letters) that identifies the application the workflow belongs to. This helps group workflows from the same application together in any list.
1. Table Schema Name: The name of the main table triggering this Workflow or Action.
1. Event Name Code(s): A short code identifying the triggering event. Combine codes when the workflow triggers on multiple events (e.g., NEWMOD for create and update).
1. Action/Purpose: Brief text describing the purpose or the action taken by the Workflow or Action.
...
1. Using short event name codes keeps the name concise while making the trigger immediately recognizable, and is consistent with the Power Automate flow naming conventions (PA-001).
...
| ---- | ----------- | -------- |
| App Prefix | App Prefix: A short prefix (3 letters) that identifies the application the workflow belongs to. This helps group workflows from the same application together in any list. | No |
| Table Schema Name | Table Schema Name: The name of the main table triggering this Workflow or Action. | No |
| Event Name Code(s) | Event Name Code(s): A short code identifying the triggering event. Combine codes when the workflow triggers on multiple events (e.g., NEWMOD for create and update). | No |
| Action/Purpose | Action/Purpose: Brief text describing the purpose or the action taken by the Workflow or Action. | No |
...
All workflows and actions must have a description
...
- "Triggered when a Contact record is created or updated. Sends an email notification to the account manager with the updated contact details. Calls the CRM - Contact - CHD - Send Notification Email child workflow."
- "Runs on demand. Cancels the selected Customer Request record and updates all related Order records to Cancelled status. Part of the Customer Request Lifecycle process."

#### Bad

- "My workflow" - meaningless, provides no context.
- "Handles stuff" - vague, no useful information.
...
Prefer asynchronous (background) workflows over real-time (synchronous) workflows unles...
...
1. Use the CHD event code in the name to clearly identify child workflows (see WF-001).
