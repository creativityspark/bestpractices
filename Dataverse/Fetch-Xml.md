...
The query above retrieves every column on the Account table, most of which are likely unused.
...
Retrieving all contacts and then filtering in code:
...
Subsequent pages use the paging cookie:
...
No page size or paging strategy — risks timeouts on tables with thousands of records and silently truncates results at the platform limit.
...
A focused query joining only what is needed:
...
An excessively deep chain of joins:
...
This query joins five tables and retrieves all columns from each, creating an expensive and hard-to-maintain query.
...
Retrieve all accounts, including those without contacts:
...
Retrieve only accounts that have at least one contact:
...
Using `inner` join when the goal is to find accounts that may or may not have contacts:
...
Using a leading wildcard prevents index usage. If an exact or prefix match is possible, prefer:
...
Also avoid chaining multiple `or` conditions when `in` is more appropriate:
...
Count of active opportunities grouped by owner:
...
An aggregate query on a large table with no filter:
...
This scans the entire activity table, which can contain millions of records and easily cause a timeout.
...
Using the SDK to convert a `QueryExpression` to FetchXML:
...
Building FetchXML by string concatenation with user input:
...
If `userInput` contains `' /><condition attribute='statecode' operator='eq' value='1`, the query logic is altered.
