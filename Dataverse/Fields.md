...
1. If the field type is Lookup, check the Relationship name and adjust it to use the [*Snake Case*](https://en.wikipedia.org/wiki/Snake_case) notation.
...
- Several fields grouped logically: `csp_review_due_date`, `csp_review_completed_on`, `csp_review_link`.

### Bad

- The same fields without a consistent prefix: `csp_due_date_review`, `csp_review_completed_on`, `csp_link_review` — harder to identify as a group.
...
1. If the field is for internal system purposes, start the description with "System Field." and include it in the hidden Admin tab ([FRM-001](/Dataverse/Forms.md#frm-001)).
...
- _"The date when the customer's last order was placed. Used to calculate inactivity alerts."_
- _"System Field. Stores the workflow processing status for automated monitoring."_

### Bad

- No description at all — users and developers have no context about the field's purpose.
- _"Field 1"_ — meaningless, provides no useful information.
